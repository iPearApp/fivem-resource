...
onNet('ipear:qrCode', (link: string) => {
    SimpleEmitter.emit('getQrCode', link); // send to NUI through our event emitter
});
...
onNet('ipear:secretCode', (code: string, expires: string) => {
    SimpleEmitter.emit('getSecretCode', code, expires); // send to NUI through our event emitter
});
